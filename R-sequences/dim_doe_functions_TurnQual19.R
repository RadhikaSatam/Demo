...
readDimDoeData<-function(ex_path){
  ...
  tryCatch(dataList$rawRelayCoeffs<-as.matrix(read.csv(paste(ex_path,"rawRelayCoeffs.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
...
{ 
  ...
  infosFromZ<-list(z_userunit=z_userunit,z_transform=z_transform,z_offset=z_offset,z_gradient=z_gradient,D_resp=D_resp,V_resp=V_resp)
  return(infosFromZ)
}
...
provide_u_design<-function(dataList,VR,u_names,u_roles,u_transform,u_offset,u_gradient,u_low,u_high,debug=0,newData=FALSE){
  ...
  if(exists("uu_design",where=dataList)&&!newData|exists("uu_new",where=dataList)&&newData) {
    ...
    if(debug) print(which(!(colnames(uu_design) %in% rownames(u_roles))))
...
#    relayDesign<-t(rD_offset+rD_gradient*t(relayDesign))   #should be raplaced by uu2u ...
...
#        relayDesign[,i]<-apply(data.frame(relayDesign[,i],rD_transform[i]),1, function(x) {if(x[2]=="none") as.numeric(x[1]) else log10(as.numeric(x[1]))} )
  ...
  if(!flag=="ok")   relayCoeffs<-list(flag=flag) else   		####change180417
    relayCoeffs<-(list(flag=flag,Rsquare=Rsquare,rDcolMeans=rDcolMeans,rDcolMiddles=rDcolMiddles,relayCoeffs=relayCoeffs))
  return(relayCoeffs)		####change180417 & 181220
}

refineRelayCoeffs<-function(rawRelayCoeffs,u_roles, u_transform, u_offset, u_gradient,u_low,u_high,debug=0)
{				#### new function change190110				
  ...
  if(!any(rXcols)|!any(rYcols)) flag<-"error: no matching columns in raw relay coeffs" else
  {
    ...
    if(normX$flag!="ok"|normY$flag!="ok") flag<-paste(normX$flag,normY$flag,sep=" and ") else
    {
      ...
      rDcolCtrs<-cbind(t(rXcolMeans),rYcolMeans)
      
#      names(rDcolMeans)[rD_roles=="contr"|rD_roles=="scup"]<-paste(colnames(X),"__N",sep="")	
#      names(rDcolMeans)[rD_roles!="contr"&rD_roles!="scup"]<-paste(colnames(Y),"__D",sep="")	
    }
  }
  if(!flag=="ok")   relayCoeffs<-list(flag=flag) else   		####change180417
...
makeRelayMatrix<-function(u_roles,relayCoeffs)
{
...
relayVmatrix<-function(u_names,u_roles,u_transform,u_offset,u_gradient,u_low,u_high,V,R=0,rDcolCtrs=0,debug=0){   ##change 190102
  flag="ok"
  ...
  new_low[names(rDcolCtrs),1]<-rDcolCtrs				#### change 190102
  ...
  new_high[setdiff(rownames(R),colnames(R)),1]<-
    (u_low[unlist(lapply(setdiff(rownames(R),colnames(R)),function(x) {unlist(strsplit(x,'__',fixed=TRUE))[1]})),1] + 
     u_high[unlist(lapply(setdiff(rownames(R),colnames(R)),function(x) {unlist(strsplit(x,'__',fixed=TRUE))[1]})),1])/2  
  new_high[names(rDcolCtrs),1]<-rDcolCtrs				#### change 190102

  if(debug) print(cbind(new_low,new_high))

  relay<-list(flag=flag,new_names=new_names,new_roles=new_roles,new_transform=new_transform,new_offset=new_offset,new_gradient=new_gradient,new_low=new_low,new_high=new_high,V=V,R=R)
  return(relay)
}

relayV_resp<-function(z_transform,z_offset,z_gradient,V_resp,R_resp=0,debug=0){
  flag="ok"
...
u2uu<-function(u_vec,u_transform,u_offset,u_gradient){
  flag<-"ok"
  uu_vec<-u_vec
