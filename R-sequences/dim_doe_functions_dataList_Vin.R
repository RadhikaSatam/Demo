...
readDimDoeData<-function(ex_path){
  ...
  tryCatch(dataList$R<-as.matrix(read.csv(paste(ex_path,"relay.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)

  tryCatch(dataList$R2<-as.matrix(read.csv(paste(ex_path,"relay2.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)

  tryCatch(dataList$relayDesign<-as.matrix(read.csv(paste(ex_path,"relayDesign.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
  ...
  tryCatch(dataList$uu_design<-as.matrix(read.csv(paste(ex_path,"uu_design.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
  ...
  tryCatch(dataList$uz_values<-as.matrix(read.csv(paste(ex_path,"uz_values.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
  ...
  tryCatch(dataList$uu_new<-as.matrix(read.csv(paste(ex_path,"uu_new.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
  ...
  tryCatch(dataList$uz_new<-as.matrix(read.csv(paste(ex_path,"uz_new.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
...
#vergl. Funktionen u2uu und uu2u (s.  unten)
  ...
  infosFromU<-list(u_roles=u_roles,u_low=u_low,u_high=u_high,u_userunit=u_userunit,u_offset=u_offset,u_gradient=u_gradient,u_transform=u_transform,D=D,V=V)
  return(infosFromU)
}
...
provide_u_design<-function(dataList,u_roles,u_transform,u_gradient,u_offset){
  if(exists("uu_design",where=dataList)) {
    ctrInd<-(1:nrow(u_roles))[u_roles=="contr"|u_roles=="eqpr"]
    u<-uu2u(dataList$uu_design,sRfM(u_transform,ctrInd),sRfM(u_gradient,ctrInd),sRfM(u_offset,ctrInd))
    if(u$flag=="ok") u_design<-u$u_vec else u_design<-u$flag
  } else if(exists("u_design",where=dataList)) u_design<-dataList$u_design else u_design<-"no design found to import"
  return(u_design)
}  
...
suggestVmatrix<-function(u_roles,D,Vin=as.matrix(0),responses=0,debug=0)
{
  ...
  if(!responses)
    {
      E2<-prepareProjection(u_roles,c("const","scup","contr"),transpose=TRUE) 
    } else { 
  ...
  if(r==n){   # impossible to make things dimensionless: return unity matrix
    ...
    V<-rbind(solve(t(sCfM(sRfM(D,1:r),1:r)),-t(sRfM(D,(r+1):n))%*%sCfM(sRfM(V,(r+1):n),1:(n-r))),sRfM(V,(r+1):n))
  ...
  if(sum(Vin^2)) {
    if(sum((t(Vin)%*%DD)^2)) { flag<-"warning: Vin is not dimensionless and is ignored" 
      ...
      repeat{
        Vnew<-cbind(Vin,V[,-sample(1:ncol(V),ncol(Vin),replace=FALSE)])
...
makeRelayMatrix<-function(relayDesign,u_roles, u_offset, u_gradient, u_transform,relayCoeffs=0,fromDesign=TRUE)
{
  ...
  if(fromDesign){
    ...
    X<-sCfM(relayDesign,rD_roles=="contr")
    Y<-sCfM(relayDesign,rD_roles!="contr")

    relayCoeffs<-solve(t(X)%*%X)%*%t(X)%*%Y
    Rsquare<-apply(Y,2,function(x) {cor(X%*%relayCoeffs,x)^2})
  }
  ...
  relayInfo<-list(relayMatrix=relayMatrix,Rsquare=Rsquare)
  return (relayInfo)
}
...
relayVmatrix<-function(u_roles,u_transform,u_gradient,u_offset,u_low,u_high,V,R=0,debug=0){
  flag="ok"
  ...
  relay<-list(flag=flag,new_roles=new_roles,new_transform=new_transform,new_gradient=new_gradient,new_offset=new_offset,new_low=new_low,new_high=new_high,V=V,R=R)
  return(relay)
}
...
relayV_resp<-function(z_transform,z_gradient,z_offset,V_resp,R_resp,debug=0){
  ...
  relay_resp<-list(new_transform=new_transform,new_gradient=new_gradient,new_offset=new_offset,V_resp=V_resp,R_resp=R_resp)
  return(relay_resp)
}
...
fetchV_resp<-function(Z,V,import,debug=0) 
{
  #Z<-dataList$Z
  rownames(Z)<-Z[,2]
  if(import) {
    ...
    respCols<-apply(V_resp[(nrow(V)+1):nrow(V_resp),],2,function(x) sum(x^2))
    V_resp<-sCfM(V_resp,(1:ncol(V_resp))[respCols==1])
    rownames(V_resp)<-rownames(E3)
...
#     then either the most suitable (cosines) or a user alternative (which) are to be used
  ...
  if(ncol(VR)>nctrR)  {                    #if too many DL-factors then slack factor(s) introduced
    if(keepCols[1]) {                      #either user wish
      ...
      DLFimportance<-apply(VR[u_roles=="contr",],2,function(x) sum(x^2))
  ...
  if(!ncol(VR)||qr(sRfM(VDS,u_roles=="contr"))$rank < ncol(VDS)){    #control factors degenerate
    ...
    in_contr<-prepareProjection(u_roles,"contr")
#VD
    VDtop<-sRfM(VDS,u_roles=="contr")
    VDbot<-sRfM(VDS,u_roles=="const"|u_roles=="dep")
...
#       RES_scup<-in_in_scup-(VDS%*%WDW)%*%in_in_scup
  ...
  #constructing RES-matrix 
    ...
    if(ncol(RES)) WRES<-rbind(WDB,solve(t(RES)%*%RES)%*%t(RES)) else
      WRES<-WDB
...
#x_designs: design DL-factors at left, then const-factors, then scup-factors, then eqpr-factors - this can be checked in x_roles
{
  ...
  if(flag=="ok") {
    ...
    designInfos<-list(u_design=u_design, x_D_low=x_D_low, x_D_high=x_D_high, MD=MD, Dfrml=Dfrml,dimDoeMod=dimDoeMod,flag=flag)
...
uu2u<-function(uu_vec,u_transform,u_gradient,u_offset){
  flag<-"ok"
  u_vec<-uu_vec
